% 2D wave equation solver: basic, initial conditions only, Neumann
% conditions
...
D1(1,1) = -1;                           % adjustments for Neumann condition
D1(N,N) = -1;
I = speye(N);
D2 = kron(D1, I)+kron(I,D1);
B = 2*speye(N^2)+lambda^2*D2;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% initial condition setup

psi0 = zeros(N,N);  % use to store initial condition profile (a lump!)
...
psi0 = reshape(psi0,N^2,1);

psi1 = psi0;            % set initial values of grid functions both to the lump
psi2 = psi0;

psi = zeros(N^2,1); % initialise unknown grid function (at the next time step)
...
for n=1:Nf
    ...
    if(plot_on)
        ...
        psiplot = reshape(psi,N,N);
        surf(([1:N]-1/2)*h, ([1:N]-1/2)*h, psiplot');
        xlabel('x (m)')
        ylabel('y(m)')
        shading interp
        axis equal
