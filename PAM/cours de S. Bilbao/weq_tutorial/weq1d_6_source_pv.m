...
rho = 1.18;             % density in kg/m^3

Tf = 0.1;               % duration of simulation (s)
xi = 0.5;               % location of source (m)
...
for n=1:Nf
    ...
    if(plot_on)
        ...
        % calculate pressure and velocity from velocity potential
        
        p_plot = (rho/k)*(psi-psi1);
        v_plot = -([psi;0]-[0;psi])/h;
        subplot(2,1,1)
        plot([1:N-1]*h, p_plot, 'k')
        xlabel('x (m)')
        ylabel('$p$ (Pa)', 'Interpreter', 'latex')
        subplot(2,1,2)
        plot(([1:N]-1/2)*h, v_plot, 'k')
        xlabel('x (m)')
